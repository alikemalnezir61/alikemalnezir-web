...
1. GitHub hesabınızda **Settings → Developer settings → OAuth Apps → New OAuth App** ile bir uygulama oluşturun.
   - Homepage URL: `https://alikemalnezir.com`
   - Authorization callback URL: `https://alikemalnezir.com/api/decap/callback`
...
4. Değişiklikleri push edip Vercel deploy'unu bekleyin, ardından `https://alikemalnezir.com/admin` adresinden GitHub hesabınızla giriş yapıp yazı ekleyebilirsiniz. Panelden yapılan her kayıt otomatik olarak GitHub'a commit atar ve Vercel yeniden yayınlar.
...
Google Search Console için: search.google.com/search-console üzerinden domain'i ekleyip DNS veya HTML dosyası ile doğrulayın, ardından `https://alikemalnezir.com/sitemap.xml` adresini gönderin.
...
- [ ] Güvenlik başlıkları canlı ortamda mevcut (`curl -I https://alikemalnezir.com` ile kontrol edilebilir)
